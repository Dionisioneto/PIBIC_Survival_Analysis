...
SpopMEP <- function(t=t, lambda.par=lambda.par,
  ...
  probY    <- 1/(1+elinpred)
  spop     <- (1-probY)+probY*S_MEP
  return(spop)
}


loglikIC.MEP.fc <- function(par, l=l, r=r, x.cure=x.cure, x.risk=x.risk, grid.vet=grid.vet){
  ...
  betas.cure = par[(b + 1):(b + n.cov.cure)]
  betas.risk = par[(b + 1 + n.cov.cure):(b + n.cov.cure + n.cov.risk)]
  ...
  p3 <- SpopMEP(t=l[cens==0], lambda.par=hazards, grid.vet=grid.vet, beta.par=betas.risk, theta.par=betas.cure, x.cure=x.cure[cens==0,], x.risk=x.risk[cens==0,])
  lik[cens==0] <- p3
